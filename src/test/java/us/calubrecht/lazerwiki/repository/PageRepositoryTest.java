...
class PageRepositoryTest {
  ...
  DbSupport dbSupport =
      new DbSupport(); // Does nothing but satisfy coverage. Constructor for utility class with only
  ...
  void test_getLastRevisionBySiteAndNamespaceAndPagename() {
    ...
    rev =
        pageRepository.getLastRevisionBySiteAndNamespaceAndPagename(
            "site1", "ns", "nonexistantPage");
  ...
  void test_findAllBySiteOrderByModifiedDesc() {
    ...
    assertEquals(
        "ns:page2#1", pageDescs.get(0).getDescriptor() + "#" + pageDescs.get(0).getRevision());
    assertEquals(
        "ns:deletedPage#2",
        pageDescs.get(1).getDescriptor() + "#" + pageDescs.get(1).getRevision());
    assertTrue(pageDescs.get(1).isDeleted());
    assertEquals(
        "ns:deletedPage#1",
        pageDescs.get(2).getDescriptor() + "#" + pageDescs.get(2).getRevision());
