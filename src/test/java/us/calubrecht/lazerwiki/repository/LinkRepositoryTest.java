...
public class LinkRepositoryTest {
  ...
  public void test_findAllBySiteAndSourcePageNSAndSourcePageName() {
    List<Link> fromHome =
        linkRepository.findAllBySiteAndSourcePageNSAndSourcePageName("default", "", "");
    assertEquals(
        Set.of("page1", "page2"),
        fromHome.stream().map(Link::getTargetPageName).collect(Collectors.toSet()));
    List<Link> fromPage1 =
        linkRepository.findAllBySiteAndSourcePageNSAndSourcePageName("default", "", "page1");
    assertEquals(
        Set.of(":page2", "ns:nsPage"),
        fromPage1.stream()
            .map(l -> l.getTargetPageNS() + ":" + l.getTargetPageName())
  ...
  public void test_findAllBySiteAndTargetPageNSAndTargetPageName() {
    List<Link> toPage2 =
        linkRepository.findAllBySiteAndTargetPageNSAndTargetPageName("default", "", "page2");
    assertEquals(
        Set.of("", "page1"),
        toPage2.stream().map(Link::getSourcePageName).collect(Collectors.toSet()));
  }
  ...
  public void test_deleteBySiteAndSourcePageNSAndSourcePageName() {
    ...
    List<Link> fromNewPage =
        linkRepository.findAllBySiteAndSourcePageNSAndSourcePageName("default", "", "newPage");
    assertEquals(
        Set.of("pageA", "pageB"),
        fromNewPage.stream().map(Link::getTargetPageName).collect(Collectors.toSet()));
