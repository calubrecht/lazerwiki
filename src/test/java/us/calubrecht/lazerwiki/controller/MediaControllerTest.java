...
class MediaControllerTest {
  ...
  void saveFile() throws Exception {
    ...
    MockMultipartFile mfile =
        new MockMultipartFile(
            "file", "someFile.jpg", MediaType.TEXT_PLAIN_VALUE, "Hello, world".getBytes());

    this.mockMvc
        .perform(multipart("/_media/upload").file(mfile).param("namespace", "ns").principal(auth))
        .andExpect(status().isOk())
  ...
  void saveFileInvalidFileType() throws Exception {
    ...
    MockMultipartFile mfile =
        new MockMultipartFile(
            "file", "someFile.js", MediaType.TEXT_PLAIN_VALUE, "Hello, world".getBytes());
  ...
  void listFiles() throws Exception {
    ...
    MediaListResponse resp =
        new MediaListResponse(Map.of("", List.of(file1, file2)), new NsNode("", true));
    when(mediaService.getAllFiles(any(), any())).thenReturn(resp);

    this.mockMvc
        .perform(get("/_media/list").principal(auth))
        .andExpect(status().isOk())
        .andExpect(
            content()
                .json(
    ...
    this.mockMvc
        .perform(
            post("/_media/moveFile")
                ...
                .principal(auth))
        .andExpect(status().isOk())
        .andExpect(content().json("{\"success\": false, \"message\": \"File already exists\"}"));
  }
}
