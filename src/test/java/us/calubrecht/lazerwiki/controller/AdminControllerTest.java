...
class AdminControllerTest {
  ...
  @MockitoBean SiteDelService siteDelService;
  ...
  void regenLinkTable() throws Exception {
    ...
    this.mockMvc
        .perform(
            post("/api/admin/regenLinkTable/default")
                .principal(new UsernamePasswordAuthenticationToken("bob", "")))
        .andExpect(status().isOk());
    this.mockMvc
        .perform(
            post("/api/admin/regenLinkTable/default")
                .principal(new UsernamePasswordAuthenticationToken("celia", "")))
        .andExpect(status().isOk());
  ...
  void regenCacheTable() throws Exception {
    ...
    this.mockMvc
        .perform(
            post("/api/admin/regenCacheTable/default")
                .principal(new UsernamePasswordAuthenticationToken("bob", "")))
        .andExpect(status().isOk());
    this.mockMvc
        .perform(
  ...
  void getUsers() throws Exception {
    when(userService.getUsers())
        .thenReturn(
            List.of(
                new UserDTO("Bob", null, List.of("ROLE_ADMIN", "ROLE_USER"), Map.of()),
                new UserDTO("Frank", null, List.of("ROLE_USER"), Map.of())));
    User adminUser = new User();
    adminUser.roles = List.of(new UserRole(adminUser, "ROLE_ADMIN"));
    when(userService.getUser("bob")).thenReturn(adminUser);
    User regularUser = new User();
    ...
    this.mockMvc
        ...
        .andExpect(
            content()
                .json(
                    "[{\"userName\":\"Bob\", \"userRoles\":[\"ROLE_ADMIN\", \"ROLE_USER\"]}, {\"userName\":\"Frank\", \"userRoles\":[\"ROLE_USER\"]}]"));

    this.mockMvc
        .perform(
            get("/api/admin/getUsers")
                .principal(new UsernamePasswordAuthenticationToken("Joe", "")))
        .andExpect(status().isOk())
        .andExpect(
            content()
                .json(
    ...
    unauthorized(
        mockMvc,
        get("/api/admin/getUsers").principal(new UsernamePasswordAuthenticationToken("frank", "")));
  }
  ...
  void deleteRole() throws Exception {
    ...
    when(userService.deleteRole(eq("Bob"), eq("ROLE_EXTRA"), any()))
        .thenReturn(new UserDTO("Bob", null, List.of("ROLE_ADMIN"), Map.of()));
    User adminUser = new User();
    adminUser.roles = List.of(new UserRole(adminUser, "ROLE_ADMIN"));
    ...
    this.mockMvc
        .perform(
            delete("/api/admin/role/Frank/ROLE_ADMIN")
                .principal(new UsernamePasswordAuthenticationToken("Bob", "")))
        .andExpect(status().isOk())
        .andExpect(content().json("{\"userName\":\"Frank\", \"userRoles\":[\"ROLE_USER\"]}"));

    // Cannot delete own ADMIN
    unauthorized(
        this.mockMvc,
        delete("/api/admin/role/Bob/ROLE_ADMIN")
            .principal(new UsernamePasswordAuthenticationToken("Bob", "")));

    // But can delete other own roles
    this.mockMvc
        .perform(
            delete("/api/admin/role/Bob/ROLE_EXTRA")
                .principal(new UsernamePasswordAuthenticationToken("Bob", "")))
        .andExpect(status().isOk())
        .andExpect(content().json("{\"userName\":\"Bob\", \"userRoles\":[\"ROLE_ADMIN\"]}"));

    // Only Admin can delete
    unauthorized(
        this.mockMvc,
        delete("/api/admin/role/Frank/ROLE_EXTRA")
            .principal(new UsernamePasswordAuthenticationToken("Frank", "")));
  }
  ...
  void addRoles() throws Exception {
    ...
    this.mockMvc
        .perform(
            put("/api/admin/role/Frank/ROLE_ADMIN")
                .principal(new UsernamePasswordAuthenticationToken("Bob", "")))
        .andExpect(status().isOk())
        .andExpect(content().json("{\"userName\":\"Frank\", \"userRoles\":[\"ROLE_USER\"]}"));
    ...
    unauthorized(
        this.mockMvc,
        put("/api/admin/role/Frank/ROLE_EXTRA")
            .principal(new UsernamePasswordAuthenticationToken("Frank", "")));
  }
  ...
  void setRoles() throws Exception {
    ...
    this.mockMvc
        .perform(
            put("/api/admin/roles/Frank/site/site1")
                .principal(new UsernamePasswordAuthenticationToken("Bob", ""))
                .content("[\"ROLE_READ:site1:bo\"]")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(content().json("{\"userName\":\"Frank\", \"userRoles\":[\"ROLE_USER\"]}"));
    verify(userService)
        .setSiteRoles(eq("Frank"), eq("site1"), eq(List.of("ROLE_READ:site1:bo")), any());
    ...
    this.mockMvc
        .perform(
            put("/api/admin/roles/Frank/site/site1")
                .principal(new UsernamePasswordAuthenticationToken("Joey", ""))
                .content("[\"ROLE_READ:site1:bo\"]")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(content().json("{\"userName\":\"Frank\", \"userRoles\":[\"ROLE_USER\"]}"));
    ...
    unauthorized(
        this.mockMvc,
        put("/api/admin/roles/Frank/site/site2")
            .principal(new UsernamePasswordAuthenticationToken("Joey", ""))
            .content("[\"ROLE_READ:site2:bo\"]")
            .contentType(MediaType.APPLICATION_JSON));
  ...
  void addUser() throws Exception {
    ...
    this.mockMvc
        .perform(
            put("/api/admin/user/NewUser")
                .content("{\"userName\": \"NewUser\", \"password\": \"password\"}")
                .contentType(MediaType.APPLICATION_JSON)
                .principal(new UsernamePasswordAuthenticationToken("Bob", "")))
        .andExpect(status().isOk())
        .andExpect(content().json("{\"userName\":\"NewUser\", \"userRoles\":[\"ROLE_USER\"]}"));
    this.mockMvc
        .perform(
            put("/api/admin/user/NewUser")
                .content("{\"userName\": \"NewUser\", \"password\": \"password\"}")
    ...
    this.mockMvc
        .perform(
            put("/api/admin/user/NewerUser")
                .content("{\"userName\": \"NewerUser\", \"password\": \"password\"}")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk());
    ...
    unauthorized(
        this.mockMvc,
        put("/api/admin/user/NewestUser")
            .content("{\"userName\": \"NewestUser\", \"password\": \"password\"}")
            .contentType(MediaType.APPLICATION_JSON));
  }
  ...
  void passwordReset() throws Exception {
    ...
    this.mockMvc
        .perform(
            post("/api/admin/passwordReset/User")
  ...
  void getSites() throws Exception {
    ...
    adminUser.userName = "Bob";
    when(userService.getUser("Bob")).thenReturn(adminUser);
    when(siteService.getAllSites(adminUser))
        .thenReturn(
            List.of(
                new Site("OneWiki", "wiki.com", "One Wiki"),
                new Site("TwoWiki", "wiki2.com", "Two Wiki")));
    this.mockMvc
        .perform(
            get("/api/admin/sites").principal(new UsernamePasswordAuthenticationToken("Bob", "")))
        .andExpect(status().isOk())
        .andExpect(
            content()
                .json(
                    "[{\"name\":\"OneWiki\", \"hostname\":\"wiki.com\", \"siteName\":\"One Wiki\"},{\"name\":\"TwoWiki\"}]"));
  }
  ...
  void addSite() throws Exception {
    ...
    regularUser.userName = "Frank";
    when(userService.getUser("Frank")).thenReturn(regularUser);
    when(siteService.getAllSites(any()))
        .thenReturn(
            List.of(
                new Site("OneWiki", "wiki.com", "One Wiki"),
                new Site("TwoWiki", "wiki2.com", "Two Wiki")));
  ...
  void setSiteSettings() throws Exception {
    ...
    this.mockMvc
        .perform(
            post("/api/admin/site/settings/TestWiki")
                .contentType(MediaType.APPLICATION_JSON)
        ...
        .andExpect(
            content()
                .json(
                    "{\"site\":{\"name\":\"TestWiki\",\"hostname\":\"wiki.com\",\"siteName\":\"Test Wiki\"},\"success\":true,\"msg\":\"\"}"));
  ...
  void setNamespaceRestrictionType() throws Exception {
    ...
    when(pageService.getAllNamespaces(eq("site1"), any())).thenReturn(response);

    this.mockMvc
        .perform(
            post("/api/admin/namespace/restrictionType")
                .contentType(MediaType.APPLICATION_JSON)
                .content(
                    "{\"site\":\"site1\", \"namespace\":\"ns\", \"restrictionType\": \"OPEN\"}")
                .principal(new UsernamePasswordAuthenticationToken("Bob", "")))
        .andExpect(status().isOk())
        .andExpect(
            content()
                .json(
                    "{\"namespaces\":{\"children\":[],\"namespace\":\"\",\"fullNamespace\":\"\"}}"));

    verify(namespaceService).setNSRestriction("site1", "ns", Namespace.RestrictionType.OPEN);
