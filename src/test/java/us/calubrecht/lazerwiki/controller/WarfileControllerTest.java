...
class WarfileControllerTest {
  ...
  void getFile() throws Exception {
    ...
    this.mockMvc.perform(get("/assets/some.unknown_filetype")).andExpect(status().isOk());
    verify(resourceService).getBinaryFile(eq("assets/some.unknown_filetype"));

    when(resourceService.getBinaryFile(eq("assets/explosive.file"))).thenThrow(new IOException(""));
    this.mockMvc.perform(get("/assets/explosive.file")).andExpect(status().isNotFound());
  }
  ...
  void getIndexFile() throws Exception {
    ...
    this.mockMvc
        ...
        .andExpect(content().string("Some bytes"));
