...
class ImportExportControllerTest {
  ...
  void exportSite() throws Exception {
    ...
    Mockito.doAnswer(
        ...
        .when(exportService)
        .createExportBundle(eq("site"), eq("Jack"), any());
    this.mockMvc
        .perform(get("/api/io/export/site").principal(auth))
  ...
  void exportSiteError() throws Exception {
    ...
    Mockito.doAnswer(
        ...
        .when(exportService)
        .createExportBundle(eq("site"), eq("Jack"), any());
    // Error during streaming is initially reported as OK. But connection closes before anything
    // sent.
    this.mockMvc
        .perform(get("/api/io/export/site").principal(auth))
        .andExpect(status().isOk())
