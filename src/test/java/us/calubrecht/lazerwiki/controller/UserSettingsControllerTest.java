...
public class UserSettingsControllerTest {
  ...
  @Autowired UserSettingsController controller;
  ...
  void test_saveEmail() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/users/saveEmail")
                .content("{\"userName\":\"jim\", \"email\":\"jimbo@super.com\"}")
                .contentType(MediaType.APPLICATION_JSON)
                .principal(new UsernamePasswordAuthenticationToken("bob", "")))
        .andExpect(status().isForbidden());

    doThrow(new RateLimitException(60))
        .when(userService)
        .requestSetEmail("Bob", "Default Site", "bob@super.com");
    mockMvc
        .perform(
            post("/api/users/saveEmail")
                .content("{\"userName\":\"Bob\", \"email\":\"bob@super.com\"}")
    ...
    mockMvc
        .perform(
            post("/api/users/resetForgottenPassword")
                .content(
                    "{\"userName\":\"Bob\", \"email\":\"bob@super.com\", \"password\":\"pass1\"}")
                .contentType(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(content().json("{\"success\":true, \"message\": \"\"}"));
    verify(userService)
        .requestResetForgottenPassword("Bob", "Default Site", "bob@super.com", "pass1");

    doThrow(new RateLimitException(300))
        .when(userService)
        .requestResetForgottenPassword("Bob", "Default Site", "bob@super.com", "pass1");
    mockMvc
        .perform(
