...
public class SiteServiceTest {
  ...
  @Autowired SiteService underTest;
  ...
  /**
   * public Object getSettingForHostname(String hostname, String setting) { Site s =
   * siteRepository.findByHostname(hostname.toLowerCase()); if (s == null ||
   * !s.settings.containsValue(setting)) { s = siteRepository.findByHostname("*"); } if (s == null )
   * {
   *
  ...
  void getAllSites() {
    ...
    assertEquals(
        List.of("BlueWiki", "FireWiki"), sites.stream().map(site -> site.siteName).toList());
  ...
  void setSiteSettings() throws SiteSettingsException {
    ...
    // non admin
    assertThrows(
        SiteSettingsException.class,
        () -> underTest.setSiteSettings("existingSite", "site", "{}", user));
    // non existent site
    assertThrows(
        SiteSettingsException.class,
        () -> underTest.setSiteSettings("noneExistentSite", "site", "{}", adminUser));

    // bad json
    assertThrows(
        SiteSettingsException.class,
        () -> underTest.setSiteSettings("existingSite", "site", "{nonKey}", adminUser));
    assertThrows(
        SiteSettingsException.class,
        () ->
            underTest.setSiteSettings(
                "existingSite", "site", "{\"key\":\"value\"}trailing", adminUser));
  }
}
