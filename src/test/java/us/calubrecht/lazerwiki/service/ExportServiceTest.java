...
class ExportServiceTest {
    ...
    @Test
    void test_createExportBunde_MissingFile() throws MediaReadException, MediaWriteException, IOException {
        when(siteService.getHostForSitename(anyString())).thenReturn("localhost");
        Map<String, List<PageDesc>> pages = Map.of(
                "", List.of(desc("", ""))
        );
        ...
        when(mediaService.getBinaryFile(anyString(), anyString(), eq("circle.png"), isNull())).thenReturn(
                getBinaryFile("circle.png"));
        when(mediaService.getBinaryFile(anyString(), anyString(), eq("ns/circleWdot.png"), isNull())).thenThrow(
                new NoSuchFileException("ns/circleWdot.png"));
    ...
    void test_createExportBunde_MediaReadException() throws MediaReadException, IOException {
        ...
        when(mediaService.getBinaryFile(anyString(), anyString(), eq("circle.png"), isNull())).thenReturn(
                getBinaryFile("circle.png"));
        when(mediaService.getBinaryFile(anyString(), anyString(), eq("ns/circleWdot.png"), isNull())).thenThrow(
                new MediaReadException("ns/circleWdot.png"));

        ByteArrayOutputStream baos = new ByteArrayOutputStream();

        assertThrows(RuntimeException.class, () -> underTest.createExportBundle("default", "george", baos));

    }

    private static int extractFiles(Path tempArchive, byte[] bytes, Path extractDir) throws IOException {
