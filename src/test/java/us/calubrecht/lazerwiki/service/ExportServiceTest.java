...
class ExportServiceTest {
  ...
  @Autowired ExportService underTest;
  ...
  public void test_createExportBundle_MissingResources() throws IOException {
    ...
    try {
      ...
      List<String> expectedPages = List.of("pages/.txt", "pages/.meta");
      for (String page : expectedPages) {
        assertTrue(
            Files.exists(extractDir.resolve(page)), "Archive did not contain [" + page + "]");
      }
      assertFalse(
          Files.exists(extractDir.resolve("resources")),
          "Archive should not include any resources");
