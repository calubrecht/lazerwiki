...
class RegenCacheServiceTest {
  ...
  @Autowired RegenCacheService underTest;
  ...
  @MockitoBean ImageRefService imageRefService;
  ...
  void test_regenLinks() {
    List<PageDesc> pds =
        List.of(
            ...
            new PageServiceTest.PageDescImpl("ns", "page2"));
    ...
    when(pageRepository.getBySiteAndNamespaceAndPagenameAndDeleted(
        ...
        .thenReturn(page1);
    when(pageRepository.getBySiteAndNamespaceAndPagenameAndDeleted(
            any(), any(), eq("page2"), eq(false)))
        .thenReturn(page2);
    when(renderer.renderWithInfo(anyString(), any(RenderContext.class)))
        .thenAnswer(
            inv -> {
  ...
  void test_regenLinksWithError() {
    ...
    ArgumentCaptor<List<String>> argument = ArgumentCaptor.forClass(List.class);
    // Even though 1st page failed continue to parse second;
    verify(linkService, times(1)).setLinksFromPage(eq("default"), any(), any(), argument.capture());
    assertEquals(List.of("page3", "ns:page4"), argument.getAllValues().get(0));
  }
  ...
  void test_regenCachesForBacklinksWithErrors() {
    ...
    assertEquals("text2 rendered", argument.getAllValues().get(0).renderedCache);
    assertEquals(true, argument.getAllValues().get(0).useCache);
    assertEquals("text3 rendered", argument.getAllValues().get(1).renderedCache);
  }
  ...
  void test_regenCachesForImageRefs() {
    ...
    when(pageRepository.getBySiteAndNamespaceAndPagenameAndDeleted("default", "", "page2", false))
        .thenReturn(page2);
    when(pageRepository.getBySiteAndNamespaceAndPagenameAndDeleted(
            "default", "ns1", "page5", false))
        .thenReturn(page5);
    when(pageRepository.getBySiteAndNamespaceAndPagenameAndDeleted(
            "default", "ns1", "page33", false))
        .thenReturn(page33);
    when(renderer.renderWithInfo(anyString(), any(RenderContext.class)))
        .thenAnswer(
            inv -> {
  ...
  void test_regenCachesForImageRefsWithErrors() {
    ...
    verify(pageCacheRepository, never()).deleteBySite("default");
    verify(pageCacheRepository, times(2)).save(argument.capture());
    assertEquals("text5 rendered", argument.getAllValues().get(0).renderedCache);
    assertEquals("text33 rendered", argument.getAllValues().get(1).renderedCache);
  }
}
