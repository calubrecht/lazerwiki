...
public class PageServiceTest {
  ...
  public void test_getPageDataWithBacklinks() {
    ...
    when(namespaceService.filterReadablePageDescriptors(any(), any(), any()))
        .thenReturn(List.of(new PageDescriptor("", "page1"), new PageDescriptor("", "page2")));
  ...
  public void test_getPageDataMoved() {
    ...
    assertEquals(
        new PageData(
            null,
            "This page has been moved to [[ns3:page3]] (ns3:page3)",
            "movedPage",
            Collections.emptyList(),
            Collections.emptyList(),
  ...
  public void test_getTemplate() {
    ...
    when(pageRepository.getBySiteAndNamespaceAndPagenameAndDeleted(
            "site", "ns1", "_template", false))
        .thenReturn(ns1Template);
    ...
    assertEquals(
        "== Page: Page 1==\nIn ns1",
        pageService.getTemplate("site", new PageDescriptor("ns1", "page1")));
    assertEquals(
        "== Page: Page 2==\nIn ns1:ns2",
        pageService.getTemplate("site", new PageDescriptor("ns1:ns2", "page2")));
    assertEquals(
        "======Page 3======", pageService.getTemplate("site", new PageDescriptor("ns3", "page3")));
    assertEquals(
        "======Page 4======", pageService.getTemplate("site", new PageDescriptor("", "page4")));
  }
  ...
  public void test_historicalPageData() {
    ...
    when(pageRepository.findBySiteAndNamespaceAndPagenameAndRevision("default", "", "page1", 1))
        .thenReturn(page);
  ...
  void test_getPageHistory() throws PageReadException {
    ...
    when(pageRepository.findAllBySiteAndNamespaceAndPagenameOrderByRevision(
            "default", "ns", "page1"))
  ...
  void test_getPageDiff() throws PageReadException {
    ...
    latestPage.setId(5L);
    when(pageRepository.getBySiteAndNamespaceAndPagename("default", "", "thisPage"))
  ...
  public void test_getPageData_bulk() {
    ...
    when(pageRepository.getAllBySiteAndNamespaceAndPagename(
            anyString(), eq("site1"), captor.capture()))
    ...
    when(namespaceService.canReadNamespace(eq("site1"), eq("ns_secret"), eq("Frank")))
        .thenReturn(false);
    res =
        pageService.getPageData(
    ...
    assertEquals(
        "You are not permissioned to read this page",
        res.get(new PageDescriptor("ns_secret", "secret")).rendered());
  ...
  void test_getAllNamespaces() {
    when(namespaceService.getReadableNamespaces(any(), any()))
        .thenReturn(List.of("", "a:c", "a:b", "a", "d:a", "d:b", "a:b:c", "a:d"));

    when(namespaceService.getNSRestriction("default", "d"))
        .thenReturn(Namespace.RestrictionType.WRITE_RESTRICTED);
    when(namespaceService.getNSRestriction("default", "d:b"))
        .thenReturn(Namespace.RestrictionType.INHERIT);

    when(namespaceService.joinNS(any(), any()))
        .thenAnswer(
            inv -> {
    ...
    assertEquals(Namespace.RestrictionType.WRITE_RESTRICTED, dTree.getRestrictionType());
    assertNull(dTree.getChildren().get(0).getRestrictionType());
    assertEquals(
        Namespace.RestrictionType.WRITE_RESTRICTED,
        dTree.getChildren().get(0).getInheritedRestrictionType());

    assertEquals("b", dTree.getChildren().get(1).getNamespace());
    assertEquals(
        Namespace.RestrictionType.WRITE_RESTRICTED,
        dTree.getChildren().get(1).getInheritedRestrictionType());
  }
