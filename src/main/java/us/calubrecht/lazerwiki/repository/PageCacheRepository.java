...
public interface PageCacheRepository extends CrudRepository<PageCache, PageCache.PageCacheKey> {
  ...
  @Query(
      value =
          "SELECT * FROM pageCache WHERE MATCH(pageName, title) AGAINST (:searchTerm IN BOOLEAN MODE) AND site=:site",
      nativeQuery = true)
  List<PageCache> searchByTitleMysql(String site, String searchTerm);

  @Query(
      value =
          "SELECT * FROM pageCache WHERE MATCH(plaintextCache) AGAINST (:searchTerm IN BOOLEAN MODE) AND site=:site",
      nativeQuery = true)
  List<PageCache> searchByTextMysql(String site, String searchTerm);

  @Query(
      value = "SELECT * FROM pageCache WHERE title MATCH :searchTerm AND site=:site",
      nativeQuery = true)
  List<PageCache> searchByTitleSqlite(String site, String searchTerm);

  @Query(
      value = "SELECT * FROM pageCache WHERE plaintextCache MATCH :searchTerm AND site=:site",
      nativeQuery = true)
