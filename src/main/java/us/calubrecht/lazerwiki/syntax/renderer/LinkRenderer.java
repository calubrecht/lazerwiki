...
public class LinkRenderer extends ContainerRenderer {
  ...
  public StringBuilder renderHtml(ITreeNode node, RenderContext renderContext) {
    ...
    if (suspiciousTarget(source)) {
      String error =
          String.format(
              "Suspicious link source at %s. Raw text =[%s]", node.getPosition().getLeft(), source);
  ...
  protected CharSequence getLinkDisplay(
      LinkNode node, String linkTarget, RenderContext renderContext) {
        ...
        ((List<LinkOverrideInstance>)
            ...
            .add(
                new LinkOverrideInstance(
                    page,
                    override,
                    link.getTargetPosition().getLeft(),
                    link.getTargetPosition().getRight() + 1));
      }
      return override;
    }
    return page;
