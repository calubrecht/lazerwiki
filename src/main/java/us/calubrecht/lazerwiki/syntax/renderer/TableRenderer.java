...
public class TableRenderer extends ContainerRenderer {
  ...
  public static class TableCellRenderer extends ContainerRenderer {
    ...
    public StringBuilder renderHtml(ITreeNode node, RenderContext renderContext) {
      ...
      String tagName = cellNode.getType() == TableNode.TableCellNode.CellType.DATA ? "td" : "th";
      String span =
          cellNode.getColSpan() > 1 ? String.format(" colspan=\"%s\"", cellNode.getColSpan()) : "";
      String align = getAlign(cellNode);
      span +=
          cellNode.getRowSpan() > 1 ? String.format(" rowspan=\"%s\"", cellNode.getRowSpan()) : "";
      StringBuilder buffer = new StringBuilder();
    ...
    public StringBuilder renderPlaintext(ITreeNode node, RenderContext renderContext) {
      ...
      StringBuilder buffer = new StringBuilder();
      buffer
          .append(source.charAt(0))
          .append(super.renderPlaintext(node, renderContext))
          .append(source.charAt(source.length() - 1));
      return buffer;
    }
  }
}
