...
public class HiddenRenderer extends ContainerRenderer {
  ...
  public StringBuilder renderHtml(ITreeNode node, RenderContext renderContext) {
    ...
    if (!rawTitle.equals(hiddenTitle)) {
      String error =
          String.format(
              "Suspicious hidden tag name at %s. Raw text =[%s]",
              hidden.getPosition().getLeft(), rawTitle);
  ...
  public StringBuilder renderPlaintext(ITreeNode node, RenderContext renderContext) {
    ...
    for (ITreeNode child : container.getChildren()) {
      buffer.append(
          parserRegistrar.getRenderer(child.getClass()).renderPlaintext(child, renderContext));
      buffer.append("\n\n");
    }
  ...
  /**
   * If hidden contain just single paragraph, extract its contents and render those as direct
   * children of hidden
   */
  public ContainerNode collapseSingleParagraph(HiddenNode node) {
    if (node.getChildren().size() == 1) {
      if (node.getChildren().get(0) instanceof TaggedContainerNode tcn
          && tcn.getType() == TaggedContainerNode.TYPE.PARAGRAPH) {
    ...
    for (String optionName : hidden.getOptions().keySet()) {
      if (!optionName.equals("name")) {
        addError(
            renderContext,
            String.format(
                "Unknown attribute \"%s\" in hidden tag at %s",
                optionName, hidden.getPosition().getLeft()));
      }
    }
  }
}
