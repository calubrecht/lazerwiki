...
public class LinkCheckMacro extends Macro {
  ...
  public String render(Macro.MacroContext context, String macroArgs) {
    ...
    if (argsMap.containsKey("filterNS")) {
      Set<String> nsBlacklist =
          Stream.of(argsMap.get("filterNS").split(","))
      ...
      Set<String> nsWhitelist =
          Stream.of(argsMap.get("ns").split(","))
              .map(String::toLowerCase)
              .map(ns -> ns.endsWith(":") ? ns : ns + ":")
              .collect(Collectors.toSet());
    ...
    if (argsMap.containsKey("filterOrphanNS")) {
      Set<String> orphanBlacklist =
          Stream.of(argsMap.get("filterOrphanNS").split(","))
              .map(String::toLowerCase)
              .map(ns -> ns.endsWith(":") ? ns : ns + ":")
              .collect(Collectors.toSet());
    ...
    Map<String, String> caseInsensitiveMapping =
        context.getAllPages().stream()
            .filter(nsFilter)
            .filter(p -> !p.endsWith(":_template") && !p.equals("_template"))
    ...
    allPages.stream()
        ...
        .forEach(
            page -> {
              ...
              linkedTo.addAll(
                  links.stream().map(String::toLowerCase).filter(allPages::contains).toList());
              links.stream()
                  .filter(l -> !allPages.contains(l.toLowerCase()))
                  .filter(context::isReadable)
                  .forEach(
                      l -> {
                        brokenLinks
                            .computeIfAbsent(l.toLowerCase(), (k) -> new ArrayList<>())
                            .add(page);
    ...
    Set<String> orphanedPages =
        allPages.stream().filter(orphanNsFilter).collect(Collectors.toSet());
    ...
    return """
                ...
                """
        .formatted(
            renderBrokenLinks(brokenLinks, caseInsensitiveMapping, brokenLinksMapping),
            renderOrphanedPages(orphanedPages, caseInsensitiveMapping));
  }

  String renderBrokenLinks(
      Map<String, List<String>> brokenLinks,
      Map<String, String> caseInsensitiveMapping,
      Map<String, String> brokenLinksMapping) {
    return brokenLinks.entrySet().stream()
        .map(
            entry ->
                "<tr><td>%s</td><td>%s</td><td>%s</td></tr>"
                    .formatted(
                        entry.getValue().size(),
                        renderLink(brokenLinksMapping.get(entry.getKey())),
                        renderLinkList(entry.getValue(), caseInsensitiveMapping)))
        .collect(Collectors.joining("\n"));
  }

  String renderOrphanedPages(
      Set<String> orphanedPages, Map<String, String> caseInsensitiveMapping) {
    return orphanedPages.stream()
        .map(page -> "<tr><td>%s</td></tr>".formatted(renderLink(caseInsensitiveMapping.get(page))))
        .collect(Collectors.joining("\n"));
  }
  ...
  String renderLinkList(List<String> pages, Map<String, String> caseInsensitiveMapping) {
    return "[%s]"
        .formatted(
            pages.stream()
                .map(p -> renderLink(caseInsensitiveMapping.get(p)))
                .collect(Collectors.joining(",")));
  }
}
