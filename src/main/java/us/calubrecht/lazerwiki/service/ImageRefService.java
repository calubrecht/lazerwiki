...
public class ImageRefService {
  ...
  @Transactional
  public void setImageRefsFromPage(
  ...
  public List<String> getImagesOnPage(String site, String page) {
    ...
    return imageRefRepository
        ...
        .map(
            ref ->
                ref.getImageNS().isBlank()
                    ? ref.getImageRef()
                    : ref.getImageNS() + ":" + ref.getImageRef())
        .collect(Collectors.toList());
  }
  ...
  public List<String> getRefsForImage(String site, String imageRef) {
    ...
    return imageRefRepository
        .findAllBySiteAndImageNSAndImageRef(site, pd.namespace(), pd.pageName())
        .stream()
        .map(
            ref ->
                ref.getSourcePageNS().isBlank()
                    ? ref.getSourcePageName()
                    : ref.getSourcePageNS() + ":" + ref.getSourcePageName())
        .collect(Collectors.toList());
  }

  public void deleteImageRefs(String site, String page) {
    PageDescriptor pd = PageService.decodeDescriptor(page);
    imageRefRepository.deleteBySiteAndSourcePageNSAndSourcePageName(
