...
public class PageUpdateService {
  ...
  public void savePage(
    ...
    if (p != null && p.getRevision() != lastRevision && !force) {
      throw new PageRevisionException(
          "Expected revision " + p.getRevision() + " but was " + lastRevision);
    }
    ...
    if (p != null) {
      ...
      action =
          p.isDeleted()
              ? ActivityType.ACTIVITY_PROTO_CREATE_PAGE
              : ActivityType.ACTIVITY_PROTO_MODIFY_PAGE;
    }
    Page newP = new Page();
    newP.setSite(site);
  ...
  public MoveStatus movePage(
    ...
    Page existingPage =
        pageRepository.getBySiteAndNamespaceAndPagename(site, newPageNS, newPageName);
    ...
    savePage(
        ...
        0,
        oldPage.getText(),
        oldPage.getTags().stream().map(PageTag::getTag).toList(),
        links,
        images,
        oldPage.getTitle(),
        user,
  ...
  @Transactional
  public boolean createDefaultSiteHomepage(String siteName, String displayName, String userName)
