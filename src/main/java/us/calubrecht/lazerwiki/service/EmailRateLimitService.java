...
public class EmailRateLimitService {
  ...
  void initCaches() {
    setEmailCache =
        Caffeine.newBuilder().expireAfterWrite(userEmailMinutes, TimeUnit.MINUTES).build();
    resetPasswordCache =
        Caffeine.newBuilder().expireAfterWrite(forgotPasswordMinutes, TimeUnit.MINUTES).build();
  }
