...
public class PageLockService {
  ...
  @Autowired PageLockRepository repository;
  ...
  public synchronized PageLockResponse getPageLock(
    ...
    if (lock == null || lock.getLockTime().isBefore(LocalDateTime.now()) || overrideLock) {
      ...
      return new PageLockResponse(
          p.namespace(), p.pageName(), revision, userName, lockTime, true, lock.getLockId());
    }
    return new PageLockResponse(
        p.namespace(),
        p.pageName(),
        revision,
        lock.getOwner().userName,
        lock.getLockTime(),
        false,
        null);
  }
