...
public class MediaService {
  ...
  @Autowired MediaCacheService mediaCacheService;
  ...
  public void saveFile(String host, String userName, MultipartFile mfile, String namespace)
    ...
    MediaRecord oldRecord =
        mediaRecordRepository.findBySiteAndNamespaceAndFileName(site, namespace, fileName);
    Long id = null;
    ...
    MediaRecord newRecord =
        new MediaRecord(
            ...
            user,
            mfile.getSize(),
            imageDimension.getRight(),
            imageDimension.getLeft());
    newRecord.setId(id);
    mediaRecordRepository.save(newRecord);
    MediaHistoryRecord historyRecord =
        new MediaHistoryRecord(fileName, site, namespace, user, action);
  ...
  @NotNull
  private File getFileInNS(String site, String nsPath, String fileName)
      throws MediaReadException, IOException {
    File f =
        nsPath.isBlank()
    ...
    if (!f.getCanonicalPath()
        .startsWith(Paths.get(rootFile.getCanonicalPath(), site, "media").toString())) {
  ...
  public MoveStatus moveImage(
    ...
    MediaRecord existingRecord =
        mediaRecordRepository.findBySiteAndNamespaceAndFileName(site, newFileNS, newFileName);
    if (existingRecord != null) {
      return new MoveStatus(false, newFileName + " already exists, move cannot overwrite it");
    }
    MediaRecord oldRecord =
        mediaRecordRepository.findBySiteAndNamespaceAndFileName(site, oldFileNS, oldFileName);
    if (oldRecord == null) {
    ...
    MediaHistoryRecord historyRecord =
        new MediaHistoryRecord(newFileName, site, newFileNS, user, action);
    mediaHistoryRepository.save(historyRecord);
  ...
  NsNode getNsNode(String site, String rootNS, List<MediaRecord> mediaRecords, String userName) {
    ...
    namespaces.forEach(ns -> nodes.add(getNsNode(site, ns, mediaRecords, userName)));
  ...
  public MediaListResponse getAllFiles(String host, String userName) {
    ...
    return new MediaListResponse(
        mediaRecords.stream().collect(Collectors.groupingBy(MediaRecord::getNamespace)),
        namespaces);
  ...
  public void deleteFile(String host, String fileName, String userName)
    ...
    MediaHistoryRecord historyRecord =
        new MediaHistoryRecord(
            fileName, site, splitFile.getLeft(), user, ActivityType.ACTIVITY_PROTO_DELETE_MEDIA);
    mediaHistoryRepository.save(historyRecord);
