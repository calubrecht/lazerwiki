...
public class HistoryController {
  ...
  List<Object> mergePageAndMedia(
      List<RecentChangesResponse.RecentChangeRec> pages, List<MediaHistoryRecord> mediaChanges) {
    ...
    while (count < 10 && !pagesStack.isEmpty() && !mediaStack.isEmpty()) {
      if (pagesStack.get(0).pageDesc().getModified().isAfter(mediaStack.get(0).getTs())) {
