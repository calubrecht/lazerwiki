...
public class AdminController {
  ...
  @Autowired SiteDelService siteDelService;
  ...
  public ResponseEntity<Void> regenLinkTable(
      @PathVariable("site") String site, Principal principal) {
  ...
  @PreAuthorize("@adminController.hasAdmin(#principal.getName(), #site)")
  public ResponseEntity<Void> regenCacheTable(
      @PathVariable("site") String site, Principal principal) {
  ...
  public ResponseEntity<UserDTO> deleteRole(
    ...
    if (user.userName.equals(userName) && userRole.equals("ROLE_ADMIN")) {
      ...
      throw new AuthorizationDeniedException(
          "Cannot remove your own admin role", new AuthorizationDecision(false));
    }
  ...
  @PreAuthorize("@adminController.hasAdmin(#principal.getName(), #site)")
  public ResponseEntity<UserDTO> setSiteRoles(
      Principal principal,
      @PathVariable("userName") String userName,
      @PathVariable("site") String site,
      @RequestBody List<String> siteRoles) {
    User user = userService.getUser(principal.getName());
    if (siteRoles.stream()
        .anyMatch(
            role -> {
  ...
  @PreAuthorize(
      "@adminController.hasSelfReg() || #principal != null && @adminController.hasRole(#principal.getName(), 'ROLE_ADMIN', 'ROLE_USERADMIN')")
  public ResponseEntity<?> addUser(
    ...
    if (userService.getUser(userName) != null) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body("User %s already exists".formatted(userName));
    }
    userService.addUser(
        userName, userRequest.password(), user, List.of(LazerWikiAuthenticationManager.USER));
    User u = userService.getUser(userName);
    UserDTO dto =
        new UserDTO(
            u.userName, null, u.roles.stream().map(role -> role.role).toList(), u.getSettings());
  ...
  @PreAuthorize("@adminController.hasAdmin(#principal.getName())")
  public ResponseEntity<List<Site>> addSite(
      Principal principal,
      @PathVariable("siteName") String siteName,
      @RequestBody SiteRequest siteRequest)
      throws PageWriteException, IOException {
    User user = userService.getUser(principal.getName());
  ...
  public ResponseEntity<SiteSettingsResponse> setSiteSettings(
      Principal principal,
      @PathVariable("siteName") String siteName,
      @RequestBody SiteSettingsRequest siteRequest) {
    User user = userService.getUser(principal.getName());
    try {
      Site site =
          siteService.setSiteSettings(
              siteName, siteRequest.hostName(), siteRequest.siteSettings(), user);
  ...
  public ResponseEntity<PageListResponse> setNamespaceRestrictionType(
      Principal principal, @RequestBody NamespaceRestrictionRequest restrictionRequest) {
    User user = userService.getUser(principal.getName());
    namespaceService.setNSRestriction(
        restrictionRequest.site(),
        restrictionRequest.namespace(),
        restrictionRequest.restrictionType());
    return ResponseEntity.ok(
        pageService.getAllNamespaces(restrictionRequest.site(), user.userName));
  }

  @DeleteMapping("site/{siteName}")
  @PreAuthorize("@adminController.hasAdmin(#principal.getName())")
  public ResponseEntity<List<Site>> deleteSite(
      Principal principal, @PathVariable("siteName") String siteName) {
    User user = userService.getUser(principal.getName());
  ...
  @PreAuthorize("@adminController.hasAdmin(#principal.getName())")
  public ResponseEntity<CommonResponse> setGlobalSettings(
      Principal principal, @RequestBody GlobalSettings settings) {
